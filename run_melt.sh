...
process_file() {
    ...
    java -Xmx8G -jar "${melt}/MELT.jar" Single -bamfile "$cram" -w "${out}/" -t "${melt}/me_refs/Hg38/LINE1_MELT.zip" -h "$fasta" -n "${melt}/add_bed_files/Hg38/Hg38.genes.bed" -c 25 -mcmq 95 #-bowtie bowtie2 -samtools samtools
...
mv "output1/LINE1.final_comp.vcf" "output/${name1}.vcf"
mv "output2/LINE1.final_comp.vcf" "output/${name2}.vcf"
